...
def patch_match(image_patches, patch):
    ...
    tiled_patches = np.zeros(image_patches.shape)
    tile = patch
    for k in range(n_patches):
        tiled_patches[:,k,:] = tile
    
    ssd = np.sum(np.sum(np.square(image_patches - tiled_patches),axis=0),axis=1)       
        
    # Find the minimum ssd and the corresponding idx
    min_ssd = min(ssd)
    min_idx = list(ssd).index(min_ssd)
